...
func Heatmap(size image.Rectangle, points []image.Point, limits image.Rectangle, dotSize int, opacity uint8,
	...
	placePoints(limits, bw, points, dot)
...
func placePoints(limits image.Rectangle,
	bw draw.Image, points []image.Point, dot draw.Image,
) {
...
func warm(out, in draw.Image, opacity uint8, colors []color.Color) {
	draw.Draw(out, out.Bounds(), image.Transparent, image.Point{X: 0, Y: 0}, draw.Src)
	bounds := in.Bounds()
...
func mkDot(size float64) draw.Image {
	...
	md := 0.5 * math.Sqrt(math.Pow(size/2.0, 2)+math.Pow((size/2.0), 2))
	for x := 0.; x < size; x++ {
		for y := 0.; y < size; y++ {
